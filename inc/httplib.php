...
if (!class_exists('HTTPRequest')) {
	...
	class HTTPRequest {
		...
		function BasicHeader($method, $params, $headerarray) {
			$crlf = "\r\n";
			if (!isset($headerarray['Host']) || !$headerarray['Host']) {
		...
		function Get($paramarray = array(), $details = false, $headerarray = array()) {
			return $this->Body(self::GET,$paramarray,$details,$headerarray);
		}
	
		//Post request
		function Post($paramarray = array(), $details = false, $headerarray = array()) {
			return $this->Body(self::POST,$paramarray,$details,$headerarray);
		}
	
		//Put request
		function Put($paramarray = array(), $details = false, $headerarray = array()) {
			return $this->Body(self::PUT,$paramarray,$details,$headerarray);
		}
	
		//Delete request
		function Delete($paramarray = array(), $details = false, $headerarray = array()) {
			return $this->Body(self::DELETE,$paramarray,$details,$headerarray);
		}
	
		function Query($method, $paramarray = array(), $details = false, $headerarray = array()) {
			$req = $this->BasicHeader($method, $this->RequestEncode($paramarray), $headerarray);
			return $this->Request($req,$details);
		}
	
		function Body($method, $paramarray = array(), $details = false, $headerarray = array()) {
			if (is_array($paramarray)) {
				$content = $this->RequestEncode($paramarray);
				if (!isset($headerarray['Content-Type']) || !$headerarray['Content-Type']) {
					$headerarray['Content-Type'] = HTTPRequest::URL_ENCODED;
				}
			} else {
				$content = $paramarray;
			}
			if (!isset($headerarray['Content-Length']) || $headerarray['Content-Length']) {
				$headerarray['Content-Length'] = $this->ContentLength($content);
			}
			$req = $this->BasicHeader($method,'',$headerarray).$content;
			return $this->Request($req,$details);
		}
		...
		function Request($req, $details = false) {
			...
			if(isset($headers['location'])) {
				...
				$req = $http->BasicHeader(self::GET, $this->RequestEncode($paramarray), $headerarray);
